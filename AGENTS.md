...
| [`context_engine.py`](context_engine.py) | R8.1-R8.3 上下文工程引擎（Phase 1 2026-08-21，**默认关**）：append-only canonical 会话（客户端历史写入期压缩后冻结，永不回溯改写——修复前缀缓存击穿）+ epoch 状态机（超 S 预算一次性收编 K 窗口外轮次入压缩区，回退保护 K=4→L3 减半→413 硬上限）；开启时管线 6/7/14/17 四 stage 联动跳过 |
...
Client POST /v1/messages（Anthropic）或 POST /v1/chat/completions（OpenAI，经 convert_openai_request_to_anthropic 转换）
  ...
  → InstrumentedPipeline 依次执行 24 个 stage（编号为代码注释中的历史层号）
      ...
      11. LoopIntervention
      12. RereadDetector
      12.5. AutoRecallStage (ctx_recall 自闭环 auto-recall, 默认关; 台账 dup 检测→manifest 折叠确认→召回尾部注入)
      13. DateNormalizer
      14. ContextTruncator
...
- **PDC 披露召回**（`ctx_recall.py` + `memory_stores.py`）：被 fifo/epoch 丢弃的单元写入 manifest（可寻址索引行），模型经 `ctx_recall` 工具查询（L1 内存子串 + L2 FTS5 trigram + archive 全文恢复），微轮自答同请求重派。**自闭环 auto-recall**（stage 12.5，`PROXY_AUTO_RECALL_ENABLED` 默认关，2026-09-05 设计）：台账 `dup_queries` 检测同 Read 目标 ≥3 次 + manifest 确认已折叠 → 代理代答取回并以 `[System: AUTO-RECALL]` user 尾消息注入（每轮至多 1 条、每会话限 5 次、同目标去重；阈值/上限/注入长度均 reloadable）——实测模型主动召回 0 调用后的主动兜底，详见 `docs/01-requirements-product/ctx-recall-auto-closed-loop-20260905.md`。
...
| `PROXY_CTX_ENGINE_ENABLED` | `false` | R8 上下文工程引擎（append-only + 写入期压缩 + epoch；开启时 6/7/14/17 跳过） |
