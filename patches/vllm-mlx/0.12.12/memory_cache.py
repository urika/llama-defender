...
class CacheStats:
    ...
    # P1a LCP snap-down（llama-defender 上下文工程 §11，2026-09-06）：混合架构
    # 条目（含 ArraysCache/Mamba 递归态）通过边界线性态检查点降档复用的计数。
    # snapdown_hits: 成功降档复用次数；snapdown_tokens_saved: 检查点位置之前的
    # 免重算 tokens。仅在 PROXY_CACHE_LCP_SNAPDOWN=1 且条目携带检查点时增长。
    snapdown_hits: int = 0
    snapdown_tokens_saved: int = 0
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            # P1a LCP snap-down observability（§11.5）：降档复用事件与免重算量。
            "snapdown_hits": int(self.snapdown_hits),
            "snapdown_tokens_saved": int(self.snapdown_tokens_saved),
        }
...
class _CacheEntry:
    ...
    # 非 trimmable（线性递归态）层的层索引与消息边界检查点。
    #   nontrim_layer_indices: 本条目 cache 中 is_trimmable()==False 层的下标
    #   linear_checkpoints: {边界位置 B: [第 i 个 nontrim 层在 B 处的状态快照]}
    # KV 层不进检查点——它们可 trim，fetch 时从条目数组直接切片（内存近零开销：
    # 只存线性态，每检查点 MB 级 vs KV 条目 GB 级）。检查点由 scheduler 在
    # prefill 跨越消息边界时经 extract_cache 捕获（递归态不可事后回卷，只能现场存）。
    nontrim_layer_indices: Any = None
    linear_checkpoints: Any = None
    ...
    def create(
        ...
        *,
        message_boundary: bool = False,
        message_boundary_sequence: int = 0,
        linear_checkpoints: dict[int, list] | None = None,
    ) -> "_CacheEntry":
        ...
        ``linear_checkpoints``（P1a §11）: {B: [nontrim 层状态]}。提供时随条目
        存储，fetch 的 LCP 分支可降档复用；内存计入 entry.memory_bytes。
        """
        memory = estimate_kv_cache_memory(cache)
        nontrim_idx = [
            i for i, layer in enumerate(cache) if _layer_is_non_trimmable(layer)
        ]
        if linear_checkpoints:
            memory += _estimate_checkpoints_memory(linear_checkpoints)
        return cls(
            tokens=tuple(tokens),
            cache=cache,
            memory_bytes=memory,
            ...
            nontrim_layer_indices=nontrim_idx,
            linear_checkpoints=linear_checkpoints,
        )


# ── P1a LCP snap-down helpers（llama-defender 上下文工程 §11，2026-09-06）────

def _env_flag(name: str) -> bool:
    """读取布尔环境开关（"1"/"true"/"yes" 为真），缺省 False。"""
    return os.environ.get(name, "").strip().lower() in ("1", "true", "yes", "on")


def snapshot_linear_states(cache: list[Any]) -> list[list[Any]]:
    """提取非 trimmable（线性递归态）层的可快照状态（供 scheduler 捕获检查点）。

    返回与 ``_layer_is_non_trimmable`` 筛选出的层一一对应的状态列表。
    递归态容器可能被原地更新（如 ArraysCache.cache 列表按槽位替换），快照
    deepcopy 容器使检查点持有私有副本；MLX 数组本身不可变，浅层引用安全。
    """
    states: list[list[Any]] = []
    for layer in cache:
        if not _layer_is_non_trimmable(layer):
            continue
        state = getattr(layer, "state", None)
        states.append(copy.deepcopy(state) if state is not None else [])
    return states


def _estimate_checkpoints_memory(checkpoints: dict[int, list]) -> int:
    """检查点内存估算：累加所有状态数组的 nbytes（线性态 MB 级 vs KV 条目 GB 级）。"""
    total = 0
    for states in checkpoints.values():
        for layer_states in states or ():
            for arr in layer_states or ():
                try:
...
def _restore_recurrent_layer(layer: Any, saved_state: list[Any]) -> Any | None:
    """从检查点恢复一个递归态层：浅拷贝层对象 + 私有容器 + 检查点数组引用。

    返回 None 表示该层类型不支持恢复（调用方放弃 snap-down，安全回退 MISS）。
    """
    import mlx.core as mx  # noqa: F401 — 保持与模块内其他延迟导入一致

    new_layer = copy.copy(layer)
    state = getattr(layer, "state", None)
    if state is None and not hasattr(layer, "state"):
        return None
    try:
        if isinstance(saved_state, list):
            new_layer.state = list(saved_state)  # 新容器，数组引用共享（不可变）
        else:
            new_layer.state = copy.copy(saved_state)
    except Exception:
        return None
    return new_layer
...
class MemoryAwarePrefixCache:
    ...
    def __init__(
        ...
        # P1a LCP snap-down（§11）：混合架构条目经边界线性态检查点降档复用。
        # 默认关——开启还需条目携带 linear_checkpoints（scheduler 多边界捕获，
        # 生产侧后续落地），本开关只控制 fetch 侧消费行为。
        self._lcp_snapdown_enabled = _env_flag("PROXY_CACHE_LCP_SNAPDOWN")
    ...
    def _build_snapdown_view(
        self, entry: "_CacheEntry", lcp: int
    ) -> tuple[list[Any], int] | None:
        """构建「检查点 B ≤ lcp」的复用视图，返回 (cache 视图, B)；失败 None（回退 MISS）。

        - KV/可 trim 层：``_trim_cache_offset`` 切片到 B（支持 QuantizedKVCache）
        - 递归层：``_restore_recurrent_layer`` 从检查点恢复（浅拷贝层 + 私有
          容器 + 不可变数组引用，绝不触碰条目本体）
        任一层失败即整体放弃——snap-down 永远是纯增益路径，失败只影响性能
        不影响正确性（回退到全量 prefill）。
        """
        ckpts = entry.linear_checkpoints or {}
        candidates = [b for b in ckpts if 0 < b <= lcp]
        if not candidates:
            return None
        b = max(candidates)
        saved_states = ckpts[b]
        nontrim_idx = entry.nontrim_layer_indices
        if nontrim_idx is None or len(nontrim_idx) != len(saved_states):
            return None  # 结构不一致（版本漂移/损坏）→ 放弃
        view: list[Any] = []
        trim_by = len(entry.tokens) - b
        si = 0
        for i, layer in enumerate(entry.cache):
            if i in nontrim_idx:
                restored = _restore_recurrent_layer(layer, saved_states[si])
                si += 1
                if restored is None:
                    return None
                view.append(restored)
            else:
                trimmed = _trim_cache_offset([layer], trim_by)
                if not trimmed or trimmed[0] is None:
                    return None
                view.append(trimmed[0])
        return view, b
    ...
    def _fetch_locked(
        ...
        if best_lcp_entry is not None and best_lcp_length > 0:
            ...
            # ── P1a LCP snap-down（llama-defender 上下文工程 §11，2026-09-06）──
            # 递归态不可回卷（ArraysCache/Mamba is_trimmable()==False 是正确
            # 行为），但边界检查点 B ≤ lcp 对共享前缀 0..B 的请求是合法续算
            # 状态：KV 层从条目数组切片到 B，递归层恢复检查点状态。
            # 仅当条目携带 linear_checkpoints（scheduler 多边界捕获）时生效；
            # 无检查点则落到下方既有路径，行为与补丁前逐字节一致。
            if (
                has_non_trimmable
                and self._lcp_snapdown_enabled
                and best_lcp_entry.linear_checkpoints
            ):
                snap = self._build_snapdown_view(best_lcp_entry, best_lcp_length)
                if snap is not None:
                    snap_cache, snap_pos = snap
                    self._entries.move_to_end(best_lcp_entry.tokens)
                    self._stats.hits += 1
                    self._stats.snapdown_hits += 1
                    self._stats.tokens_saved += snap_pos
                    self._stats.snapdown_tokens_saved += snap_pos
                    self._last_match_type = "lcp_snapdown"
                    remaining = tokens[snap_pos:]
                    logger.info(
                        "[cache_fetch] LCP snapdown: B=%d reused=%d remaining=%d "
                        "(entry_len=%d lcp=%d)",
                        snap_pos, snap_pos, len(remaining),
                        len(best_lcp_entry.tokens), best_lcp_length,
                    )
                    snap_cache = self._decompress_cache(snap_cache)
                    return snap_cache, remaining
