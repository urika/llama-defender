...
## 9A. 实测进展（2026-09-07 更新）

- 框架扩至 **32 案例/9 套件**（+TC22/23 引擎快速验证、TC24 真实模型探针、
  TC25-27 per-route、TC28a/b huge 豁免、TC29 行为金标准、TC30 代码目标
  auto-recall、TC31 政策钉住），**31/31 全绿**（TC29 需 --real-backend）；
- DEF-310/311/312/313 治理与观测落地：行号前缀修复（AST 解锁）、
  IFC-11 反编造 cue、IFC-12 事实钉住、IFC-13 断言核对、卡死转径、
  huge 视图口径豁免；
- 生产重启一次拾取至 DEF-308 轨道②前的状态；符号召回/TC28/卡死转径/
  IFC-11 cue 等后续批次待下次重启；
- per-route 豁免已实现（TC25/26/27 全绿），§10.1 状态同步为已落地。
