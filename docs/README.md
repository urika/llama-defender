...
| `business-cases-validation-20260830.md` | 业务案例与系统解决方案推演（**产品验证**）：五个业务案例（防死循环/wiki 综述更新/会话健康监控/多文件代码修复/压缩成本评估）逐一推演三层架构+五协议的端到端方案——用户故事/验收标准/协议调用链/数据流经的契约/预计效果；防死循环效率提升~180x(20s vs 61min)；协议覆盖矩阵(P4 Recall+Signal 覆盖 4/5 最高优先)；验证状态(锚充分性待测/可测升级待调/无损分解待正式实验) |
| `prd-cognitive-orchestrator-v4.md` | PRD v4.0：认知编排器（**开发输入**）——R11-R15 五大需求（编排器核心/数据契约/设计模式/并发性能/运维治理）；核心指标（死循环检测 15s / wiki 更新 80% / 预警前 3-5 轮 / 代码修复 100% 原子 / 压缩汇率 R²≥0.4）；三期架构图+五协议调用流+数据流；技术约束（Py3.9/stdlib/扁平/线程安全/reloadable）；五批次交付计划（A 契约→B Recall+Escalate→C Decompose+编排→D 模式+并发→E 后治理）约 12 天 |
...
| `three-layer-architecture-spec-20260830.md` | 三层架构正式规范（**落地蓝图 v1.0**）：OSI→TCP/IP 式简化——理论五层压缩为落地三层(Signal/Protocol/Application)+跨层基础设施；五个核心协议完整规范——P1 Decompose(双判据递归)、P2 Execute(小步协议 V=H)、P3 Verify(三级:机械/语义/对账)、P4 Recall(FTS5+子串双层检索)、P5 Escalate(决策表+幂等闸)；**完整数据契约**——14 个 TypedDict 定义(SignalSnapshot/Task/SubTask/Patch/Citation/Verdict/RecallQuery/RecallResult/EscalationDecision/ManifestLine/SessionState 等)含类型关系图；ProtocolOrchestrator 编排伪码；与现有代码映射(Signal✅/Recall✅未挂载/Decompose🔴/Escalate🔴/编排器🔴)；测试策略；配置参数表；非功能需求(fail-open/有界/可观测/可逆/stdlib/线程安全) |
...
| `cognitive-gap-closure-design-20260901.md` | 认知空白点收口设计（**未实施**，DeepSeek 元认知对话对照分析产物）：四缺口四方案——SEM 语义卡蒸馏（LLM 自蒸馏情景→语义记忆，FTS5 L0 检索注入）/ LRC 循环退火回滚（冷却半衰防振荡+triggers 升级+发送视图尾部截除回滚+escalate 决策表首个真实消费点）/ HG HealthGate 前置门（IFC 事后信号劫持既有 L2/L3 干预路径，监控信号优先于数据信号，不做真抢占）/ TAC 工具准入契约（contract_registry 运行时三查：大小走 PDC 恢复链+错误喂 blocker+schema fail-open 记账回填）；四 Wave 观测先行路线与阶段门、17 项 CONFIG_REGISTRY 新增、测试锚点与不做清单 |
| `skill-progressive-disclosure-insights-20260901.md` | Skill 渐进式信息披露机制分析与压缩借鉴（**调研提案，未实施**）：四级披露栈（L1 元数据常驻 description 路由键 / L2 主体按触发页入 / L3 资源二跳引用 / L4 脚本零内容加载——代码即行动）+ 三原理（驻留/激活成本解耦、模型即路由器、可再生优于已持有）；五条借鉴——manifest 索引行触发词工程（拉取命中率上限由触发词密度决定）、**可再生优先压缩重排**（unit_model 加 replayable：可再生丢掉留再生命令指针=严格零失真，配 dup 守卫防 TokenSieve 重读死循环复发）、ctx_recall list 模式（模型自主翻目录分页）、PDC-L1 摘要二跳地址、skill 失败模式镜像对照表；四借鉴落地形态与优先级（P1-P3）、验收信号与配置约束 |
| `protocol-layer-ownership-review-20260902.md` | Protocol Layer 归属与边界设计 Review（v1.2 经 agent_go 反馈**定版**：核心结论"实现正确、归属错误"维持——任务工程归 agent_go、输入工程归代理；**Phase 2 由"整包迁移 7 模块"改"组件拆解吸收"**：编排器冻结为可执行规范、characterization 基线 124 个随行、全部吸收后归档删除，验收标准=机械前置层+确定性 replan+共享 EscalationDecision；B1 已关闭（agent_go 纯 Python，共享契约包形态）；C2 软化为 LRC-P3 改写轮次级独立升级策略；新端点 signals/task-context/pin 按 **R17-R19 纳入集成契约版本化**；双端任务分工 LD-1..8/AG-1..8；M1-M6+盲点 B1-B5 全记录） |
| `rag-kv-storage-fit-survey-20260901.md` | RAG 组件与 KV 存储匹配度调研（**选型文档已据此修订**）：向量存储（sqlite-vec 上游停滞→三级降级链 FTS5-only→纯 Python 暴力余弦→社区 fork）/ 编排框架（LangChain 系全排除）/ 记忆框架（mem0/Letta/Zep 2026 格局反证自建轻量路线）/ KV 存储（SQLite 甜点区保持，LMDB/Redis 排除）/ KV cache 卸载（LMCache/Mooncake 消歧不适用）；**核心结论：存储引擎不是瓶颈、embedding 生产方式才是**（sidecar 违不变式/云端有成本隐私/蒸馏零基建优先），SEM-P2 前置决策项 |
