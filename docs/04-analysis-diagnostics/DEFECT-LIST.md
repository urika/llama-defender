...
### DEF-310: 模型层幻觉——折叠后事实性提问自信编造值（模型行为层，P2） — 🔴 已定性待治理（2026-09-06）
...
| **治理矩阵（四侧）** | ①**供给侧**：auto-recall 代理代答（EXP recall-r2 H3 验证中，绕过 C 环）／关键事实 auto-pin（登记时模式识别 token/key/密钥类内容 → 常驻窗口，复用 R19 pin 机制）／折叠面板升级结论账本（已列折叠改造批）②**提示侧**：RECALL_CUE 追加反编造禁令（"召回不到必须明示，禁止编造值"）+ cue 尾部显著性——TC29 即 A/B 量测仪 ③**验证侧**：代理断言值核对——模型断言具体值时与 orig/manifest 比对，不匹配下轮注入更正（复用 rewrite_ctx_recall_results 注入模式）④**观测侧**：TC29 周期金丝雀——幻觉率/召回率进趋势指标，模型/提示变更后必测 |
...
| 🔴 未修复/待治理 | 2 | 6% | DEF-306 (fail-closed), DEF-310 (模型层幻觉, 治理矩阵已定) |
