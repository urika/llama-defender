...
> **端点官方校准（2026-08-15）**：zhipu=`https://open.bigmodel.cn/api/paas/v4`（docs.bigmodel.cn 确认，key 冒烟有效，429/1113=余额不足需充值；temperature 区间 (0,1) 不支持 0）。kimi=**`https://api.kimi.com/coding/v1`**（Kimi Code 会员订阅体系，kimi.com/code/docs 确认，真实调用 200；`api.kimi.com` 与 `api.moonshot.cn` 是两套独立体系/计费；模型名与 Anthropic 端点同名：k3/kimi-for-coding(-highspeed)，能力实测自 `/coding/v1/models`：k3=1048576 ctx、thinking only、vision）。moonshot=`https://api.moonshot.cn/v1`（开放平台按量计费，platform.kimi.com 确认，模型名 kimi-k3/kimi-k2.7-code(-highspeed)/kimi-k2.6；key 未配→fallback 链自动跳过；本机直连超时待网络侧确认）。
